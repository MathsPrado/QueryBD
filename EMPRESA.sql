...
group by pais
/*2.*/
select sum(preco) as soma,avg(preco) as media,max(preco) as maiorpreco,min(preco) as menorpreco 
from produtos
/*3.*/
...
/*4.*/
select nome,sobrenome,cargo,salario,(salario*1.10) as salario_novo
from Funcionarios
/*5.*/
select sum(dp.preco) as somadosprecos
from detalhesped dp, pedidos p 
where(dp.NumPed=p.numped) and (year(p.dataentrega)=1997) and (month(p.dataentrega)=5)

/*6.*/
select c.codcli, c.nome,c.pais
from clientes c ,pedidos p
where (c.CodCli=p.codcli) and (year(p.dataped)=1997) and (month(p.dataped)=09)
...
where Pais = 'EUA'
UNION
Select Contato as 'Contato', Cargo as 'Cargo', 'Fornecedores' as tipo
from Fornecedores
where Pais = 'EUA'
...
where Pais = 'Brasil'
UNION
select Nome as 'Nome', Contato as 'Contato', Pais as 'País', 'Clientes' as tipo
from Clientes
where Pais = 'Alemanha'
...
where Pais = 'Reino Unido'
UNION 
Select Contato as 'Contato', Cargo as 'Cargo', Pais as 'Pais', 'Fornecedores' as tipo
from Fornecedores
where Pais = 'França'
/*--------------------------------------------------------------*/
 --Subconsulta Introduzidas com In e Not in
...
SELECT DISTINCT Nome, Contato, Cargo from Clientes c
Inner Join Pedidos P on C.CodCli = P.CodCli
where P.DataPed between '1996-08-01' and '1996-08-31'

--------------------------------------------------------------------------------------	
---outra forma

select DISTINCT c.nome, c.contato, c.cargo
FROM Clientes c, Pedidos p
where c.CodCli = p.CodCli and
p.DataPed Between '1996-08-01' and '1996-08-31'
